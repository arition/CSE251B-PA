...
Pre_train_model_model = models.resnet18(pretrained=True)


optimizer = optim.Adam(Pre_train_model_model.parameters(), lr=0.0001)

use_gpu = torch.cuda.is_available()
if use_gpu:
    Pre_train_model_model = Pre_train_model_model.cuda()


def train():
    last_iter = 0
    for epoch in range(epochs):
        Pre_train_model_model.train()
        ts = time.time()
        for iter, (X, Y) in enumerate(train_loader):
            ...
            outputs = Pre_train_model_model(inputs)
        ...
        torch.save(Pre_train_model_model, 'latest_model')
...
def val(epoch):
    ...
    Pre_train_model_model.eval()  # Don't forget to put in eval mode !
    ts = time.time()
    with torch.no_grad():
        for iter, (X, Y) in enumerate(val_loader):
            ...
            ts = time.time()
            outputs = F.log_softmax(Pre_train_model_model(inputs), dim=1)
