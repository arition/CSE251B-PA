...
Glorot and Bengio proposed their own weight initialization strategy, which they referred to as “normalized initialization” in their paper, and which is now popularly termed “Xavier initialization.”
...
For a layer of the network with d-dimensional input, $ x=(x^{(1)},...,x^{(d)})$ $ x=(x^{(1)},...,x^{(d)})$, each dimension of its input is then normalized (i.e. re-centered and re-scaled) separately,
...
\begin{equation}
\text { where } k \in[1, d] \text { and } i \in[1, m] ; \mu_{B}^{(k)} \text { and } \sigma_{B}^{(k)^{2}} \text { are the per-dimension mean and variance, respectively. }
\end{equation}

$\epsilon$ is added in the denominator for numerical stability and is an arbitrarily small constant. The resulting normalized activation $ {\hat {x}}^{(k)}$$ {\hat {x}}^{(k)}$have zero mean and unit variance, if $\epsilon$  is not taken into account. To restore the representation power of the network, a transformation step then follows as

Formally, the operation that implements batch normalization is a transform 
\begin{equation}
B N_{\gamma^{(k)}, \beta^{(k)}}: x_{1 \ldots m}^{(k)} \rightarrow y_{1 \ldots m}^{(k)}
\end{equation}
called the Batch Normalizing transform. The output of the BN transform
\begin{equation}
y^{(k)}=B N_{\gamma^{(k)}, \beta^{(k)}}\left(x^{(k)}\right)
\end{equation}
is then passed to other network layers, while the normalized output $ {\hat {x}}_{i}^{(k)}$$ {\hat {x}}_{i}^{(k)}$ remains internal to the current layer.

\begin{equation}
\begin{aligned}
&y_{i}^{(k)}=\gamma^{(k)} \hat{x}_{i}^{(k)}+\beta^{(k)}\\
&\text { where the parameters } \gamma^{(k)} \text { and } \beta^{(k)} \text { are subsequently learned in the optimization process. }
\end{aligned}
\end{equation}
%--------------------------------------------
...
solution, but this could give us a flavor of how the complete network looks like. The structure of custom model is shown in
\autoref{table:unet_model}. We maintain exactly the same structure except the output channel is modified to our n\_class.

\begin{table}[h]
	\caption{Unet model}
	\label{table:unet_model}
	\centering
	\hspace*{-1.2cm}\begin{tabular}{@{}llllllll@{}}
		\toprule
		Layer             & in\_Ch &out\_Ch     &kernal size    &stride    & padding          \\
		\midrule	
		Conv\_Block        & 3     & 32      &3   & 1   & 1    \\
		Conv\_Block             & 32    & 64         &3   &1   & 1     \\
		Conv\_Block             & 64    & 128        &3   &1   & 1     \\
		Conv\_Block             & 128   & 256        &3   &1   & 1     \\
		Conv\_Block             & 256   & 512        &3   &1   & 1     \\
		Conv\_Block             & 512   & 1024       &3   & 1   & 1     \\
		deConv\_Block             & 1024   & 512       &3   & 1   & 1     \\
		deConv\_Block             & 512   & 256       &3   & 1   & 1     \\
		deConv\_Block             & 256   & 128       &3   & 1   & 1     \\
		deConv\_Block             & 128   & 64       &3   &1  & 1     \\
		deConv\_Block             & 64   & 32       &3   &1  & 1     \\
		deConv\_Block             & 32   & n\_class       &3   &1   & 1     \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{table}[h]
  ...
  \hspace*{-1.2cm}\begin{tabular}{@{}llllllll@{}}
    ...
    weighted loss     & 0.8052         & 0.3444   & 0.8648   & 0.2406       & 0.5104  & 0.2967        & 0.9472  \\
