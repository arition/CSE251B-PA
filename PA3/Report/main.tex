...
\subsection{U-Net}
The U net shows potential to beat the FCN model, however, the model is too large, we have to resize the model to 720 * 1280 and only use SGD (mini bathch size = 1) for training, and we only run 20 epochs, so the result is not that good, however we found that the precision is still increasing at the 20th epoch, so we believe that with more epochs the model would perform much better than current result. The loss is shown in \autoref{fig:loss_val_Unet} and final result is shown in \autoref{fig:visualization_Unet}.
