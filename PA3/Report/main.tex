...
For the baseline model, we achieved a relateed high accuracy. But when we were trying to do some data augmentation, the model performance did not increase as what we expected. For the mirror flip method, the performance was even worse than the baseline model. For the rotation method, we were trying to randomly rotate the image by a small degrees (-5,5), the performance increase a little bit but somehow was only by 1-2 \%. For the imbalanced class, the typical way would be trying to introduce the weighted loss by accommodate the frequency of each class over all the training dataset. 
%--------------------------------------------
