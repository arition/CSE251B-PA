...
optimizer = optim.Adam(MyModel_net.parameters(), lr=0.0001)

use_gpu = torch.cuda.is_available()
if use_gpu:
    MyModel_net = MyModel_net.cuda()
...
def train():
    ...
    for epoch in range(epochs):
        ...
        torch.save(MyModel_net, 'latest_model')
...
def val(epoch):
    ...
    MyModel_net.eval()  # Don't forget to put in eval mode !
    ts = time.time()
    with torch.no_grad():
        for iter, (X, Y) in enumerate(val_loader):
            ...
            ts = time.time()
            outputs = F.log_softmax(MyModel_net(inputs), dim=1)
