...
The gold standard for the confirmation of line and tube positions is chest radiographs. Once getting
...
radiologists can be busy reporting other scans. The delays would cause severe complications in most of the cases.
Leveraging deep learning, we believe that we could develop algorithms that may be able to detect mispositioned catheters and lines automatically and in  a much sooner fashion.  
Once alerted from the model, clinicians can reposition or remove them immediatedly to avoid life-threatening complications, which will dramatically reduce the human resources involved.

Under the COVID-19 pandemic, almost all the hospitals are at capacity, and more patients require correctly positioned
catheter lines and tubes. If successful, our efforts may help clinicians save lives and release a lot of human resources on other emergency. 
Quick feedback on catheter and line placement could help clinicians better treat these patients. 
Even beyond COVID-19, detection of line and tube position will always be required by many ill hospital patients.
...
In\cite{wang2018chestnet}, Hongyu Wang and Yong Xia proposed a model called ChestNet. It consists of two branches.
The first classification branch is a uniform feature extraction-classification network, 
which would, to some extent, free users from troublesome handcrafted feature extraction.
The second branch is an attention branch to exploit the correlation between class labels and the locations of patholog-ical abnormalities.
By introducing the attention branch, the patholog-ically abnormal regions could be adaptively concentrated by the model
With this model they achieved SOTA on the Chest X-ray 14 dataset.
...
each layer to every other layer in a feed-forward fashion. For the traditional convolotional networks, usually it would have L layers with L connections. 
But for the DenseNet, it has L(L+1)/2 direct connections. 
Each layer in the DenseNet takes the feature-maps of all proceding layers as the inputs and all its own feature-maps would be used as the inputs into all subsequent layers.
Not only does the DenseNets alleviate the vanishing-gradient problem, but also strengthen feature propagation, encourage feature reuse, and substantially reduce the number of parameters.
...
The overall AUC for differrnt heads shown in \autoref{fig:head_val_overall}.
...
In this report, we tried four different backbones: ResNet50, DenseNet201, EfficientNet-b4 and EfficientNet-b5. From \autoref{fig:cnn_val_overall}, we can see that EfficientNet-b5 has the highest AUC. It is expected since it is the
...
We tried two different headers: one with Linear layer and the other with CNN + Linear layer. Comparing AUC for different heads using data in \autoref{fig:head_val_overall}, we can see that CNN + Linear layer head
...
From \autoref{fig:cnn_categories_val_auc}, we could see that, we achieved a relatively high accuracy on ETT, NGT and SGC (above 90\% accuracy, some of them even above 95\% accuracy). 
But for the CVC, the highest accuracy we could achieve is around 79\%. 
From \autoref{fig:visual_tubes}, we could see that CVC is the most complicated situation. We could have the tubes positioned on the left side, right side or even both sides of the body.
And we could also have multiple tubes in one patient. It would be very hard for the network to correctly detect all different CVC cases. 
For the other types of tubes, they are relatively straightforward, which match with our network performances.

\begin{figure}[h]

    \centering
    \includegraphics[width=0.98\textwidth]{Visualization.png}
     \caption{Visualization of different types of tubes.}
