...
\begin{abstract}
	In this report, we proposed our solution to the Kaggle competition "RANZCR CLiP - Catheter and Line Position Challenge." We used different architectures like Resnet, EfficientNet, DenseNet as the backbone of our solution to classify the line and tube from x-ray photos. We also applied different image augmentation methods to enhance our results. We introduced the learning rate scheduling method for fast convergence during the training, and we also employed mixed-precision training to accelerate the whole procedure while saving the GPU memory. The experiment result shows that...
\end{abstract}
%--------------------------------------------
