...
\subsection{(b)}
The loss of the validation set is shown in \ref{fig:1}. And the accuracy on test set is around 63.33\%, the validation loss
clearly shows the overfiting problem here that loss goes down quickly at first then goes up. Here we have 29 PCA
components. Looking at the PCA components(the first 4 are shown in \ref{fig:2}), we found that since the objects are not
...
achieved here is 83.5\% with 300 epochs. The mean and std of accuracy and loss is shown in \ref{fig:3}. We also try
different laearning rate here, one is too large, one is too small and a normal one, the difference is shown in
\ref{fig:4}. As we can see that a large learning rate may lead to strong oscillation while a small one would lead to
...
powerful enough to generalized to similar objects. The mean and std of training loss is shown in \ref{fig:5}.
...
The top four principal components is shown in \ref{fig:6}. From the principal components we can see that they are
different kinds of cars. The confusion matrix of the test set is shown in \ref{fig:7}. From the confusion matrix we
can see that our classfier is not very accurate. We archived 58\% accuracy on the test set. Loss and accuracy for
training and holdout data is shown \ref{fig:8}.
...
the loss curve of batch gradient decent and stochastic gradient descent is shown in \ref{fig:9}. From our result there
...
The weights for four car types are shown in \ref{fig:10}. The images of four weights roughly shows the outline of each
